...
class Tads3Lexer(RegexLexer):
    ...
    def _make_string_state(triple, dbl, verbatim=None, _escape=_escape):
        if verbatim:
            verbatim = ''.join(['(?:%s|%s)' % (re.escape(c.lower()),
                                               re.escape(c.upper()))
                                for c in verbatim])
        ch = r'"' if dbl else r"'"
        token = String.Double if dbl else String.Single
        escaped_quotes = r'+|%s(?!%s{2})' % (ch, ch) if triple else r''
        prefix = '%s%s' % ('t' if triple else '', 'd' if dbl else 's')
        tag_state_name = '%sqt' % prefix
        state = []
        if triple:
            state += [
                (r'%s{3,}' % ch, token, '#pop'),
                (r'\\%s+' % ch, String.Escape),
                (ch, token)
            ]
        else:
            state.append((ch, token, '#pop'))
        state += [
            Include('s/verbatim'),
            (r'[^\\<&{}%s]+' % ch, token)
        ]
        ...
        state += [
            Include('s/escape'),
            (r'\{([^}<\\%s]|<(?!<)|\\%s%s|%s|\\.)*\}' %
             (ch, ch, escaped_quotes, _escape), String.Interpol),
            (r'[\\&{}<]', token)
        ]
        return state

    def _make_tag_state(triple, dbl, _escape=_escape):
        ch = r'"' if dbl else r"'"
        quantifier = r'{3,}' if triple else r''
        state_name = '%s%sqt' % ('t' if triple else '', 'd' if dbl else 's')
        token = String.Double if dbl else String.Single
        escaped_quotes = r'+|%s(?!%s{2})' % (ch, ch) if triple else r''
        return [
            (r'%s%s' % (ch, quantifier), token, '#pop:2'),
            (r'(\s|\\\n)+', Text),
            ...
            (r'\{([^}<\\%s]|<(?!<)|\\%s%s|%s|\\.)*\}' %
             (ch, ch, escaped_quotes, _escape), String.Interpol),
            (r'([^\s=><\\%s]|<(?!<)|\\%s%s|%s|\\.)+' %
             (ch, ch, escaped_quotes, _escape), Name.Attribute),
            Include('s/escape'),
            Include('s/verbatim'),
            Include('s/entity'),
            (r'[\\{}&]', Name.Attribute)
