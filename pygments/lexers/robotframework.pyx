...
class VariableSplitter:
    ...
    def _split(self, string):
        ...
        self._string = string
        start_index += 2
        for index, ch in enumerate(string[start_index:]):
            index += start_index  # Giving start to enumerate only in Py 2.6+
            try:
                self._state(ch, index)
    ...
    def _variable_state(self, ch, index):
        self._variable_chars.append(ch)
        if ch == '}' and not self._is_escaped(self._string, index):
            self._open_curly -= 1
            if self._open_curly == 0:
                if not self._is_list_or_dict_variable():
                    raise StopIteration
                self._state = self._waiting_list_variable_index_state
        elif ch in self._identifiers:
            self._state = self._internal_variable_start_state

    def _is_list_or_dict_variable(self):
        return self._variable_chars[0] in ('@','&')

    def _internal_variable_start_state(self, ch, index):
        self._state = self._variable_state
        if ch == '{':
            self._variable_chars.append(ch)
            self._open_curly += 1
            ...
            self._variable_state(ch, index)

    def _waiting_list_variable_index_state(self, ch, index):
        if ch != '[':
            raise StopIteration
        self._list_and_dict_variable_index_chars.append(ch)
        self._state = self._list_variable_index_state

    def _list_variable_index_state(self, ch, index):
        self._list_and_dict_variable_index_chars.append(ch)
        if ch == ']':
            raise StopIteration
